...
const CategorSelection = ({onSelectCategory , activeCategory}) => {
    const categories = [ "Nature" , "Technologies"]
  return (
    <div className='px-4 mb-8 lg:space-x-16
    flex flex-wrap items items-center border-b-2 py-5 text-gray-900'>
      <button onClick={() => onSelectCategory(null)} className={`lg:ml-12 ${activeCategory ? "" : "active-button"}`}>All</button>
      {
        categories.map((category) => (
            <button
            onClick={() => onSelectCategory(category)}
            className={`mr-2 space-x-16 to-blue-700 ${activeCategory === category ? "active-button" : ""}`}
            key={category}>
            {category}</button>
        ))
      }
